...
class PPO(Algorithm):
    def __init__(
        ...
        self.buffer = RolloutBuffer(
            buffer_size=rollout_length, mix=mix_buffer, device=self.device)
    ...
    def save_models(self, step, env_id, save_path=f"{PACKAGE_PATH}/model", last_score=0):
        ...
        torch.save(self.critic_old.state_dict(
    ...
    def load(self, step, env_id, load_path=f"{PACKAGE_PATH}/model", last_score=0):
        ...
        self.critic_old.load_state_dict(torch.load(
            critic_load_path, map_location=lambda storage, loc: storage))
        self.actor_old.load_state_dict(torch.load(
            actor_load_path, map_location=lambda storage, loc: storage))
...
class PPOExpert(PPO):
    ...
    def __init__(self, state_dim, action_dim, net_width, device, actor_path, critic_path):
        ...
        self.critic_old = Critic(
            state_dim=state_dim, net_width=net_width).float().to(device)
