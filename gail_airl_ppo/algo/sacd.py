...
from ..network.policy import CategoricalPolicy
from ..torchkit.networks import FlattenMlp
from ..torchkit import pytorch_utils as ptu

# This version of SAC-discrete is not compatible with the Alogirhtm class from
# gail_airl_ppo/algo/base.py

# It should be treated as a helper class for Model-free off-policy separate RNN
# which is compatible with the Algorithm class from gail_airl_ppo/algo/base.py
...
class SACD():
    ...
    def __init__(
        ...
        if self.automatic_entropy_tuning:
            ...
            self.alpha_entropy_optim = Adam(
                [self.log_alpha_entropy], lr=alpha_lr)
    ...
    def critic_loss(
        ...
        with torch.no_grad():
            ...
            # q_target: (T, B, 1)
            q_target = rewards + (1.0 - dones) * gamma * \
                min_next_q_target  # next q
    ...
    def actor_loss(
        ...
        # E_{a\sim \pi}[Q(h,a)]
        policy_loss = (new_probs * policy_loss).sum(axis=-
                                                    1, keepdims=True)  # (T+1,B,1)
        if not markov_critic:
