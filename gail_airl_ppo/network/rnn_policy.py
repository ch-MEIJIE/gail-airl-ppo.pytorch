...
from ..torchkit.constant import GRU_name, RNNs
...
class Actor_RNN(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if self.image_encoder is None:
            self.observ_embedder = FeatureExtractor(
        ...
        if self.image_encoder is None:
            self.current_observ_embedder = FeatureExtractor(
                obs_dim, observ_embedding_size, F.relu
            )
    ...
    def get_hidden_states(
        ...
        if initial_internal_state is None:  # initial_internal_state is zeros
            ...
            output, current_internal_state = self.rnn(
    ...
    def forward(self, prev_actions, rewards, observs):
        ...
        joint_embeds = torch.cat(
            (hidden_states, curr_embed), dim=-1)  # (T+1, B, dim)
    ...
    def get_initial_info(self):
        ...
        hidden_state = ptu.zeros(
            (self.num_layers, 1, self.rnn_hidden_size)).float()
    ...
    def act(
        ...
        # 3. joint embed
        joint_embeds = torch.cat(
            (hidden_state, curr_embed), dim=-1)  # (1, B, dim)
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.algo.continuous_action and self.image_encoder is None:
            ...
            self.current_shortcut_embedder = FeatureExtractor(
